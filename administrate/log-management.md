...
To do so, Elasticsearch provides a functionnality called [Index Lifecycle management](https://www.elastic.co/guide/en/elasticsearch/reference/current/index-lifecycle-management.html) that allows you to create a policy to delete old indexes based on their old age.

With our Elasticsearch add-on, we automatically create a Kibana application in which you can create the policy and apply it to your indexes with an index template.

Here is a policy example that will delete indexes older than 30 days:
...
Then create an index template to apply the policy:
